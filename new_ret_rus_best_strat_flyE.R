...
{    m <- 1  
     ...
     up <- temp_p3*3
     for (p3 in low:up) {  
       for (percent in c(0.5) ){
         for (p1 in 1:UP1 ){   
           for (p2 in 0:UP2 ){  
           #вектор дельт    
           temp <- new_ret(p1, p2, p3, STEP, N, resultDataFull, UP1, UP2, percent) 
           return.winner<- new_ret.winner(p1, p2, p3, STEP, N, resultDataFull, UP1, UP2, percent) 
           return.loser<- new_ret.loser(p1, p2, p3, STEP, N, resultDataFull, UP1, UP2, percent) 
           n <- length(temp)
